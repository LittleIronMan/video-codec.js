...
var OpenH264Decoder = /** @class */ (function () {
    function OpenH264Decoder(worker) {
        var _this = this;
        this.worker = worker;
        ...
        this.decoder = Module.getValue(handle, 'i32');
        Module._free(handle);
        if (ret != 0)
        ...
        this.buf = Module.HEAPU8.subarray(this.buf_ptr, this.buf_ptr + this.MAX_NALU_SIZE);
        this.sbi_ptr = Module._malloc(_SizeOfSBufferInfo());
        this.out_ptr = Module._malloc(4 * 3);
        this.out = Module.HEAP32.subarray(this.out_ptr / 4, this.out_ptr / 4 + 3);
        this.worker.onmessage = function (e) {
            _this._setup();
        };
    }
    OpenH264Decoder.prototype._setup = function () {
        var _this = this;
        this.worker.onmessage = function (e) {
            _this._decode(e.data.data);
        };
        this.worker.postMessage({ status: 0 });
    };
    OpenH264Decoder.prototype._decode = function (data) {
        this.buf.set(new Uint8Array(data));
        this.out[0] = this.out[1] = this.out[2] = 0;
        var ret = _WelsDecoderDecodeFrame(this.decoder, this.buf_ptr, data.byteLength, this.out_ptr, this.sbi_ptr);
        if (ret != 0) {
            this.worker.postMessage({ status: ret });
            return;
        }
    ...
    OpenH264Decoder.prototype._parse_sbuffer_info = function () {
        var sbi = this.sbi_ptr;
        var in_ts_lo = Module.getValue(sbi + 4, 'i32');
        var in_ts_hi = Module.getValue(sbi + 8, 'i32');
        var out_ts_lo = Module.getValue(sbi + 12, 'i32');
        var out_ts_hi = Module.getValue(sbi + 16, 'i32');
        return {
            buffer_status: Module.getValue(sbi, 'i32'),
            input_bs_timestamp: in_ts_lo | (in_ts_hi << 32),
            output_yuv_timestamp: out_ts_lo | (out_ts_hi << 32),
            width: Module.getValue(sbi + 24, 'i32'),
            height: Module.getValue(sbi + 28, 'i32'),
            strides: [
                Module.getValue(sbi + 36, 'i32'),
                Module.getValue(sbi + 40, 'i32'),
            ]
        };
    };
    return OpenH264Decoder;
}());
